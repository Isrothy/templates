...
Vector det(const Vector &A, const Vector &B) { return (Vector){A.y * B.z - A.z * B.y, A.z * B.x - A.x * B.z, A.x * B.y - A.y * B.x}; }
...
int segment_segment_intersection(const Point &A, const Point &B, const Point &C, const Point &D, Point &O) {
...
double triangle_triangle_distrance(Point *T1, Point *T2) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            res = min(res, segment_segment_distance(T1[i], T1[(i + 1) % 3], T2[j], T2[(j + 1) % 3]));
        }
    }
    return res;
}
