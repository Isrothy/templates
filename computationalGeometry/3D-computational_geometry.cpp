struct Point {
    ...
    Point operator/(const double &p) const { return (Point){x / p, y / p, z / p}; }
    bool operator==(const Point &_) const { return dcmp(x, _.x) == 0 && dcmp(y, _.y) == 0 && dcmp(z, _.z) == 0; }
...
int line_line_intersection(const Point &A, const Point &B, const Point &C, const Point &D, Point &O) {
    if (!coplaner(A, B, C, D) || dcmp(det(B - A, D - C).len2()) == 0) { return 0; }
...
bool line_segment_triangle_intersection(const Point &A, const Point &B, const Point &P0, const Point &P1, const Point &P2, Point &P) {
    if (line_segment_line_segment_intersection(A, B, P0, P1, P)) { return true; }
    if (line_segment_line_segment_intersection(A, B, P1, P2, P)) { return true; }
    if (line_segment_line_segment_intersection(A, B, P2, P0, P)) { return true; }
    Vector n = det(P1 - P0, P2 - P0);
    if (dcmp(dot(n, B - A)) == 0) { return false; }
    double t = dot(n, P0 - A) / dot(n, B - A);
    if (dcmp(t) < 0 || 0 < dcmp(t, 1)) { return false; }
    P = A + (B - A) * t;
...
bool triangle_triangle_intersection(Point *T1, Point *T2) {
    Point P;
    for (int i = 0; i < 3; ++i) {
        if (line_segment_triangle_intersection(T1[i], T1[(i + 1) % 3], T2[0], T2[1], T2[2], P)) { return true; }
        if (line_segment_triangle_intersection(T2[i], T2[(i + 1) % 3], T1[0], T1[1], T1[2], P)) { return true; }
    }
    return false;
}
