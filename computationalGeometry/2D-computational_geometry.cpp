...
struct Point {
    ...
    Point operator+(Point const &_) const { return (Point){x + _.x, y + _.y}; }
    Point operator-(Point const &_) const { return (Point){x - _.x, y - _.y}; }
    Point operator*(double p) const { return (Point){x * p, y * p}; }
    Point operator/(double p) const { return (Point){x / p, y / p}; }
    bool operator==(Point const &_) const { return sign(x, _.x) == 0 && sign(y, _.y) == 0; }
    Point unit() const { return *this / len(); }
    double angle() const { return atan2(y, x); }
    Point normal() const { return (Point){-y, x}; }
};
typedef Point Vector;
Point operator*(double p, Point const &_) { return (Point){p * _.x, p * _.y}; }
double dot(Vector const &A, Vector const &B) { return A.x * B.x + A.y * B.y; }
double det(Vector const &A, Vector const &B) { return A.x * B.y - A.y * B.x; }
Point middle(Point const &A, Point const &B) { return 0.5 * (A + B); }
...
int intersection(Point const &A, Point const &B, Point const &C, Point const &D, Point &O) {
    if (sign(det(B - A, D - C)) == 0) { return 0; }
    double s1 = det(D - A, C - A);
    double s2 = det(C - B, D - B);
    O = A + (B - A) * (s1 / (s1 + s2));
    return 1;
}
int segment_segment_intersection( Point const &A, Point const &B, Point const &C, Point const &D, Point &O) {
    if (!intersection(A, B, C, D, O)) return 0;
...
double circumscribed_circle(Point const &A, Point const &B, Point const &C, Point &O) {
    if (intersection(middle(A, B), middle(A, B) + (A - B).normal(), middle(B, C), middle(B, C) + (B - C).normal(), O) == 0) { return -1; }
    return (A - O).len();
}
...
double external_co_tangent( Point const &O1, double r1, Point const &O2, double r2, Point &P1, Point &P2, Point &P3, Point &P4) {
    if (r1 < r2) { return external_co_tangent(O2, r2, O1, r1, P3, P4, P1, P2); }
