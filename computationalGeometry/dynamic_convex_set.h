...
struct dynamic_convex_set {
    ...
    std::set<Point, cmp1> L;
    std::set<Point, cmp2> U;
    template<typename T> bool contain(T &S, Point const &P) {
        if (S.size() < 2) {
            return false;
        }
    ...
    template<typename T> void insert(T &S, Point const &P) {
        if (contain(S, P)) {
            return;
        }
        S.insert(P);
