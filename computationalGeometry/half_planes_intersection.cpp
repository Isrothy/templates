int half_planes_intersection(pair<Point, Point> *A, Point *B, int n) {
    ...
    Q[r++] = A[0];
    for (int i = 1; i < n; ++i) {
        if (dcmp(det(Q[r - 1].first - Q[r - 1].second, A[i].first - A[i].second)) == 0){ continue; }
        while (r - l > 1 && dcmp(det(A[i].first - t[r - 1], A[i].second - t[r - 1])) <= 0) { --r; }
        while (r - l > 1 && dcmp(det(A[i].first - t[l + 1], A[i].second - t[l + 1])) <= 0) { ++l; }
        intersection(Q[r - 1].first, Q[r - 1].second, A[i].first, A[i].second, t[r]);
        Q[r++] = A[i];
    }
    while (r - l > 1 && dcmp(det(Q[l].first - t[r - 1], Q[l].second - t[r - 1])) <= 0) { --r; }
