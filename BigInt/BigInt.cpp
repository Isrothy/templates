...
#define PLACE_HOLDER "%09d"
    ...
    explicit BigUnsigned(const char *s) {
        for (int i = (int) strlen(s); i > 0; i -= bit) {
            int x = 0;
            for (int j = std::max(i - bit, 0); j < i; ++j) {
                x = x * 10 + s[j] - '0';
            }
            push_back(x);
        }
...
#undef PLACE_HOLDER
    ...
    bool operator<(const BigUnsigned &that) const {
        return cmp(that) < 0;
    }
    bool operator>(const BigUnsigned &that) const {
        return cmp(that) > 0;
    ...
    static std::pair<BigUnsigned, BigUnsigned> divide(BigUnsigned A, BigUnsigned b) {
        ...
        for (int i = (int) a.size(); i >= b.size(); --i) {
            ...
            while (a[i - 1] > b.back() + 1) {
                try_devide(a[i - 1] / (b.back() + 1));
            }
            bool left = true;
...
struct BigInt {
    ...
    BigInt(bool _isNegative, BigUnsigned _digits)
        : _is_negative(_isNegative), _digits(std::move(_digits)) {}
        ...
        if (is_negative() == that.is_negative()) {
            ...
            if (is_negative()) {
                if (digits() <= that.digits())
                    return {false, that.digits() - digits()};
                else
                    return {true, digits() - that.digits()};
            } else {
