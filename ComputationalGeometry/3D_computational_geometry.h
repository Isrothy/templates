...
struct Point {
    ...
    Point operator/(const double &p) const { return (Point){x / p, y / p, z / p}; }
    bool operator==(const Point &_) const { return sign(x - _.x) == 0 && sign(y - _.y) == 0 && sign(z - _.z) == 0; }
    double operator/(const Point &_) const {
        if (unit() == _.unit()) { return len() / _.len(); }
        return -len() / _.len();
    }
...
using Vector = Point;
Point operator*(const double &p, const Point &A) { return (Point){A.x * p, A.y * p, A.z * p}; }
...
int point_on_segment(const Point &P, const Point &A, const Point &B) { return (int) (sign(det(A - P, B - P).len2()) == 0 && sign(dot(A - P, B - P)) <= 0); }
...
auto point_plane_diatance(const Point &P, const Point &P0, const Vector &n) { return fabs(dot(P - P0, n)); }
auto point_plane_projection(const Point &P, const Point &P0, const Vector &n) { return P - n * dot(P - P0, n); }
auto coplaner(const Point &A, const Point &B, const Point &C, const Point &D) { return (int) (sign(det(det(C - A, D - A), det(C - B, D - B)).len2()) == 0); }
auto line_line_intersection(const Point &A, const Point &B, const Point &C, const Point &D, Point &O) {
    if (!coplaner(A, B, C, D) || sign(det(B - A, D - C).len2()) == 0) { return 0; }
    Vector s1 = det(D - A, C - A);
    Vector s2 = det(C - B, D - B);
    O = A + (B - A) * (s1 / (s1 + s2));
    return 1;
}
auto segment_segment_intersection(const Point &A, const Point &B, const Point &C, const Point &D, Point &O) {
    if (!line_line_intersection(A, B, C, D, O)) { return 0; }
...
auto segment_segment_distance(const Point &P1, const Point &P2, const Point &Q1, const Point &Q2) {
    ...
    if (0 <= sign(s) && sign(s - 1) <= 0 && 0 <= sign(t) && sign(t - 1) <= 0) { return ((P1 + s * (P2 - P1)) - (Q1 + t * (Q2 - Q1))).len(); }
...
auto point_triangle_distance(const Point &P, const Point &A, const Point &B, const Point &C) {
    ...
    if (point_in_triangle(P0, A, B, C)) { return (P - P0).len(); }
    return std::min({point_segment_distance(P, A, B), point_segment_distance(P, A, C), point_segment_distance(P, B, C)});
}
bool segment_triangle_intersection(const Point &A, const Point &B, const Point &P0, const Point &P1, const Point &P2, Point &P) {
    if (segment_segment_intersection(A, B, P0, P1, P)) { return true; }
    if (segment_segment_intersection(A, B, P1, P2, P)) { return true; }
    if (segment_segment_intersection(A, B, P2, P0, P)) { return true; }
    Vector n = det(P1 - P0, P2 - P0);
    if (sign(dot(n, B - A)) == 0) { return false; }
    double t = dot(n, P0 - A) / dot(n, B - A);
    if (sign(t) < 0 || 0 < sign(t - 1)) { return false; }
    P = A + (B - A) * t;
...
bool triangle_triangle_intersection(Point T1[3], Point T2[3]) {
    Point P{};
    for (int i = 0; i < 3; ++i) {
        if (segment_triangle_intersection(T1[i], T1[(i + 1) % 3], T2[0], T2[1], T2[2], P)) { return true; }
        if (segment_triangle_intersection(T2[i], T2[(i + 1) % 3], T1[0], T1[1], T1[2], P)) { return true; }
    }
    return false;
}
double triangle_triangle_distrance(Point T1[3], Point T2[3]) {
    if (triangle_triangle_intersection(T1, T2)) { return 0; }
    double res = DBL_MAX;
    for (int i = 0; i < 3; ++i) {
        res = std::min(res, point_triangle_distance(T1[i], T2[0], T2[1], T2[2]));
        res = std::min(res, point_triangle_distance(T2[i], T1[0], T1[1], T1[2]));
        for (int j = 0; j < 3; ++j) { res = std::min(res, segment_segment_distance(T1[i], T1[(i + 1) % 3], T2[j], T2[(j + 1) % 3])); }
    }
    return res;
}
