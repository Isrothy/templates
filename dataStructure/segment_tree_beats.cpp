...
void push_up(int p) {
    ...
    } else { second_max[p] = max(second_max[p], mx[p << 1]); }
    if (mx[p] == mx[p << 1 | 1]) { cnt_max[p] += cnt_max[p << 1 | 1];
    } else { second_max[p] = max(second_max[p], mx[p << 1 | 1]); }
    if (mi[p] == mi[p << 1]) { cnt_min[p] += cnt_min[p << 1];
    } else { second_min[p] = min(second_min[p], mi[p << 1]); }
    if (mi[p] == mi[p << 1 | 1]) { cnt_min[p] += cnt_min[p << 1 | 1];
    } else { second_min[p] = min(second_min[p], mi[p << 1 | 1]); }
}
...
void check_max(int p, int x) {
    ...
    if (mx[p] == mi[p]) { mx[p] = lazy_min[p] = x;
    } else if (second_max[p] == mi[p]) { second_max[p] = x; }
    mi[p] = lazy_max[p] = x;
}
void check_min(int p, int x) {
    if (mx[p] <= x) { return; }
    sum[p] -= (long long) cnt_max[p] * (mx[p] - x);
    if (mx[p] == mi[p]) { mi[p] = lazy_max[p] = x;
    } else if (second_min[p] == mx[p]) { second_min[p] = x; }
    mx[p] = lazy_min[p] = x;
}
...
void update_add(int p, int l, int r, int a, int b, int x) {
    ...
    int mid = (l + r) >> 1;
    push_down(p);
    if (b <= mid) { update_add(p << 1, l, mid, a, b, x);
    } else if (mid < a) { update_add(p << 1 | 1, mid + 1, r, a, b, x);
    } else {
...
void check_max(int p, int l, int r, int a, int b, int x) {
    ...
    int mid = (l + r) >> 1;
    push_down(p);
    if (b <= mid) { check_max(p << 1, l, mid, a, b, x);
    } else if (mid < a) { check_max(p << 1 | 1, mid + 1, r, a, b, x);
    } else {
...
void check_min(int p, int l, int r, int a, int b, int x) {
    ...
    int mid = (l + r) >> 1;
    push_down(p);
    if (b <= mid) { check_min(p << 1, l, mid, a, b, x);
    } else if (mid < a) { check_min(p << 1 | 1, mid + 1, r, a, b, x);
    } else {
